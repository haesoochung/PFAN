        ...
        with tf.Session() as sess:
            ...
            for iteration in [self.saved_iter] :
                ...
                for n in range(1, nscene+1):
                    scene_path = '%s/%03d' % (self.input_path,n)
                    ...
                    if adjust:
                        if self.center_crop:
                            gt_HDR_ = center_crop(gt_HDR_, self.IMG_WIDTH, self.IMG_HEIGHT)
                        else:
                            gt_HDR_ = cv2.resize(gt_HDR_, (self.IMG_WIDTH,self.IMG_HEIGHT))

                    batch_input_LDR_low_tensor, batch_input_LDR_mid_tensor, batch_input_LDR_high_tensor, batch_input_HDR_low_tensor, batch_input_HDR_mid_tensor, batch_input_HDR_high_tensor = make_test_batch(image_path, info_path, self.IMG_WIDTH, self.IMG_HEIGHT, self.center_crop)
                      
                    tic = time.clock()                     
                    output_coarse, mask, output_fine, output = sess.run([self.MODEL.output_coarse,self.MODEL.mask,self.MODEL.output_fine, self.MODEL.G], 
                                                        feed_dict={
                    ...
                    out_coarse_hdr = output_coarse[0,:,:,::-1].astype(np.float32)
                    out_fine_hdr = output_fine[0,:,:,::-1].astype(np.float32)                    
                    gt_HDR_ = gt_HDR_[:,:,::-1].astype(np.float32)
                    out_hdr = np.clip(out_hdr, 0., 1.)   

                    output_coarse_tm = tonemap(output_coarse[0,:,:,::-1].astype(np.float32)) * 255.
                    output_fine_tm = tonemap(output_fine[0,:,:,::-1].astype(np.float32)) * 255.
                    ...
                    PSNRs.append(psnrT)
                    hdr_path = '%s/%03d_chung_hdr_%06d.hdr' % (self.output_path, n, iteration)
                    gt_path = '%s/%03d_chung_gt.hdr' % (self.output_path, n)
                    cv2.imwrite(hdr_path, out_hdr)
                    cv2.imwrite(gt_path, gt_HDR_) 
...
def tonemap(img):
  return np.log(1 + 5000.*(img)) / np.log(1+5000.)
