...
class Model(object):    
    ...
    def Align(self, input_to_align, ref, is_train, scope):
        with tf.variable_scope(scope) :
            ...
            ref_1 = self.resden_fft_conv_block(ref_1, ch, is_train, scope, "resden1")
            
            ref_2 = self.conv_layer(ref_1, scope, name="layer2_conv1", kshape=[3,3,ch,ch], strides=[1,2,2,1])
            ref_2 = self.resden_fft_conv_block(ref_2, ch, is_train, scope, "resden2")
            ...
            sup_1 = self.resden_fft_conv_block(sup_1, ch, is_train, scope, "resden1")
            ...
            sup_2 = self.resden_fft_conv_block(sup_2, ch, is_train, scope, "resden2")
            ...
            kernels = tf.reshape(k_conv1, [-1, h, w, ch, self.kernel_size*self.kernel_size]) # 8 256 256 3 25 
            ...
            def_conv_1 = tf.reduce_sum(interpolation(ref_2, offsets, ch) * kernels, axis=-1) # b h w 64
            ...
            kernels_x2 = tf.reshape(k_conv2, [-1, 2*h, 2*w, ch, self.kernel_size*self.kernel_size]) # 8 256 256 3 25 
            ...
            def_conv_2 = tf.reduce_sum(interpolation(ref_1, offsets_x2, ch) * kernels_x2, axis=-1) # b h w 64
    ...
    def generate_mask_1(self, x, thr=0.95):
        x = tf.log(1 + 5000.*(x)) / tf.log(1+5000.)
        self.cond = tf.expand_dims(tf.cast(tf.less(tf.reduce_max(x,-1), thr), tf.float32), -1)
        return 1-self.cond
        # return sat
    ...
    def Merge(self, input_low, input_mid, input_high, is_train, scope):
        with tf.variable_scope(scope) :
            ...
            for i in range(5) :
                inter = self.res_fft_conv_block(inter, ch, is_train, scope, "resb%d"%(i))
            ...
            self.hard_mask = self.generate_mask_1(self.output_coarse, 0.9)
            self.error_map = self.mask
    ...
    def res_fft_conv_block(self, bottom, bsize, is_train, scope, name):
        res = self.conv_layer(bottom, scope, name=name+"_conv1", kshape=[3,3,bsize,bsize], strides=[1,1,1,1])
        res = tf.nn.relu(res)
        res = self.conv_layer(res, scope, name=name+"_conv2", kshape=[3,3,bsize,bsize], strides=[1,1,1,1])

        fft = tf.transpose(bottom, [0,3,1,2])
        fft = tf.signal.rfft2d(fft) 
        a = tf.real(fft)
        b = tf.imag(fft)
        fft = tf.concat([a,b], axis=1) 
        fft = tf.transpose(fft, [0,2,3,1]) 
        fft = self.conv_layer(fft, scope, name=name+"_fft_conv1", kshape=[1,1,2*bsize,2*bsize], strides=[1,1,1,1])
        fft = tf.nn.relu(fft)
        fft = self.conv_layer(fft, scope, name=name+"_fft_conv2", kshape=[1,1,2*bsize,2*bsize], strides=[1,1,1,1])
        fft = tf.transpose(fft, [0,3,1,2])
        a = fft[:, :bsize, :, :]
        b = fft[:, bsize:, :, :]
        fft = tf.complex(a, b) 
        fft = tf.signal.irfft2d(fft)
        fft = tf.transpose(fft, [0,2,3,1])

        return bottom + res + fft

    def resden_fft_conv_block(self, y_0, bsize, is_train, scope, name):
        ...
        y_7 = tf.concat([y_0, y_1, y_2],3) 
        res = self.conv_layer(y_7, scope, name=name+"_conv7", kshape=[1,1,bsize+bsize*2,bsize], strides=[1,1,1,1])

        fft = tf.transpose(y_0, [0,3,1,2]) 
        shape = fft.get_shape().as_list()
        fft = tf.signal.rfft2d(fft) 
        a = tf.real(fft)
        b = tf.imag(fft)
        fft = tf.concat([a,b], axis=1) 
        fft = tf.transpose(fft, [0,2,3,1]) 
        y_1 = self.conv_layer(fft, scope, name=name+"_fft_conv1", kshape=[1,1,2*bsize+bsize*0,2*bsize], strides=[1,1,1,1])
        y_1 = self.lrelu(y_1, scope, "conv1_lrelu1")

        y_2 = tf.concat([fft, y_1],3) 
        y_2 = self.conv_layer(y_2, scope, name=name+"_fft_conv2", kshape=[1,1,2*bsize+2*bsize*1,2*bsize], strides=[1,1,1,1])
        y_2 = self.lrelu(y_2, scope, "conv2_lrelu1")

        y_7 = tf.concat([fft, y_1, y_2],3) 
        y_7 = self.conv_layer(y_7, scope, name=name+"_fft_conv7", kshape=[1,1,2*bsize+2*bsize*2,2*bsize], strides=[1,1,1,1])
        fft = tf.transpose(y_7, [0,3,1,2]) 
        a = fft[:, :bsize, :, :]
        b = fft[:, bsize:, :, :]
        fft = tf.complex(a, b) 
        fft = tf.signal.irfft2d(fft, fft_length=[shape[2],shape[3]])
        fft = tf.transpose(fft, [0,2,3,1]) 
    ...
    def upsample2(self, bottom, scope, name, factor=[2,2]):
        size = [int(bottom.shape[1] * factor[0]), int(bottom.shape[2] * factor[1])]
        with tf.name_scope(scope+"_"+name):
            return tf.image.resize_bicubic(bottom, size=size, align_corners=None, name=None)
    ...
    def conv_3d_layer(self, bottom, scope, name, kshape, strides=[1, 1, 1, 1, 1], is_train=True):
        with tf.variable_scope(scope+"_"+name):
            ...
            out = tf.nn.conv3d(bottom,W,strides=strides, padding='SAME')
