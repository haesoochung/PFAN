...
class Train(object):
    def __init__(self, affix, saved_iter, size, batch_size, learning_rate, max_epoch, tfrecord_path, checkpoint_dir, num_of_data, step_disp):
    ...
    def define_loss(self):      
        ...
        self.msfr_loss = content_loss('msfr', self.output_coarse_tm, self.gt_HDR_tm)\
                     + content_loss('msfr', self.output_fine_tm, self.gt_HDR_tm) 
            
        self.g_loss = self.tm_l2_loss + self.cosine_loss + 0.1*self.msfr_loss + 0.001*self.vgg_loss + 0.1*self.tv_loss
            ...
            while True:
                try:
                    ...
                    if step % self.step_disp == 0:
                        G, g_loss, tm_l2_loss, lr, summary = sess.run([self.MODEL.G, self.g_loss, self.tm_l2_loss, self.learning_rate, self.merged_summary_op] \
                                                                    , feed_dict={
                                                                       self.input_LDR_low    : bt_inLl,
                                                                       self.input_LDR_mid    : bt_inLm,
                                                                       self.input_LDR_high   : bt_inLh,
